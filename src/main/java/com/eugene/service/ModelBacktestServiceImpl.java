...
//    private static final String BASE_PATH = "/Users/eugene/Downloads/data/";
    ...
    @Override
    public HurstModelResult doHurstModel(HurstModelParameters paras) {
        HurstModelResult res = new HurstModelResult();
        try {
            Process p = Runtime.getRuntime().exec(PY_PATH+" "+HURST_PY_FILE_PATH+" "+BASE_PATH
                    +" "+paras.getInstrument()+" "+paras.getStartDay()+" "
                    +paras.getEndDay()+" "+paras.getWindow());

            BufferedReader in = new BufferedReader(new InputStreamReader(p.getInputStream()));
            res.setDates(in.readLine());
            res.setClose(in.readLine());
            res.setHurst(in.readLine());
    ...
    private static final String MA_INSTRUMENT = "000001.SH";
    private static final String START_DAY = "2013-01-01";
    private static final String END_DAY = "2015-06-06";
...
//            System.out.println(res.getAccuracy());
    ...
    private static void hurstTest() {
        HurstModelResult res = new HurstModelResult();
        try {
            Process p = Runtime.getRuntime().exec(PY_PATH+" "+HURST_PY_FILE_PATH+" "+BASE_PATH
                    +" "+MA_INSTRUMENT+" "+START_DAY+" "+END_DAY+" "+233);
